...
<div class="grid grid-cols-4">
  {#each materials as material, i (i)}
    <div
      class="relative flex flex-col items-start gap-4 p-4 m-4 text-left card"
    >
      ...
      {#if material.genres.length > 0}
        <div class="grid grid-cols-2 gap-4 p-4 card variant-soft-surface">
          <h3 class="col-span-2 h3">Genres</h3>
          {#each material.genres as genre}
            <div
              class="px-4 py-2 text-center variant-soft-secondary rounded-3xl"
            >
              {genre}
            </div>
          {/each}
      ...
      {/if}
      <div
        class="grid justify-between w-full grid-cols-2 gap-4 mt-auto mb-0 ml-auto mr-0 card variant-soft"
      >
        <a
